...
if contains(subject, "rest") && not(contains(subject, "restart"))
    ...
    % ★★★ ここから修正/追加 ★★★
% filepartsで抽出したパスが空でなければ処理
    if ~isempty(output_dir)
    % -pオプションは、存在しない親フォルダ（csv/26_yamashita/など）も同時に作成します
        [success, message, ~] = mkdir(output_dir); 
    
    if ~success
        % フォルダ作成に失敗した場合はメッセージを表示
        error('フォルダ作成に失敗しました: %s', message);
    end
end
% ★★★ 修正/追加ここまで ★★★
    writecell(C,path)% 出力
    ...
    [output_dir1, ~, ~] = fileparts(path1);
    
    % フォルダを作成（path2のフォルダはpath1と同じなので、path1だけでOK）
    if ~isempty(output_dir1)
        [success, message, ~] = mkdir(output_dir1); 
        if ~success
            error('フォルダ作成に失敗しました (PLI/Concentration): %s', message);
        end
    end
...
function folder_name = findFolderName(subject)
    ...
    elseif contains(subject, "ishida")
        folder_name = "31_ishida";
    else
        folder_name = "Unknown_Subject"; 
        warning('findFolderName:SubjectNotFound', '警告: 被験者名 "%s" は未登録です。フォルダ名 "Unknown_Subject" を使います。', subject);
    end
end
